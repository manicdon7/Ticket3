...
def detail(request):
    ...
    date = request.POST.get(ids[2])  # Convert to int
    time = request.POST.get(ids[3])  # Convert to int
    location = request.POST.get(ids[4])
    total_tickets = request.POST.get(ids[5])  # Convert to int
    ticket_price = request.POST.get(ids[6]) # Convert to int
    ...
    greeting_transaction = simple_storage.functions.createEvent(
        name, description, date, time, total_tickets, ticket_price
    ).buildTransaction(
        {
...
def host(request):
    a = detail(request)
    return render(request, "host.html",a)
